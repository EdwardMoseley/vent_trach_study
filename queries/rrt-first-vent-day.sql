...
DROP MATERIALIZED VIEW IF EXISTS rrtfirstventday CASCADE;
CREATE MATERIALIZED VIEW rrtfirstventday as
with cv as
(
  ...
  inner join chartevents ce
    ...
    and ce.charttime between (ie.first_vent_starttime - interval '6' hour) and (ie.first_vent_starttime + interval '1' day)
...
(
  ...
  inner join inputevents_mv tt
    ...
    and tt.starttime between (ie.first_vent_starttime - interval '6' hour) and (ie.first_vent_starttime + interval '1' day)
    and itemid in
    (
...
(
  ...
  inner join datetimeevents tt
    on ie.icustay_id = tt.icustay_id
    and tt.charttime between (ie.first_vent_starttime - interval '6' hour) and (ie.first_vent_starttime + interval '1' day)
    and itemid in
    (
