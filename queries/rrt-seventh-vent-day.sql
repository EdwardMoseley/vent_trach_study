...
DROP MATERIALIZED VIEW IF EXISTS rrtseventhventday CASCADE;
CREATE MATERIALIZED VIEW rrtseventhventday as
with cv as
(
  ...
  inner join chartevents ce
    ...
    and ce.charttime between (ie.first_vent_starttime - interval '6' hour + interval '7 days') and (ie.first_vent_starttime + interval '8 days')
...
(
  ...
  inner join inputevents_mv tt
    ...
    and tt.starttime between  (ie.first_vent_starttime - interval '6' hour + interval '7 days') and (ie.first_vent_starttime + interval '8 days')
    and itemid in
    (
...
(
  ...
  inner join datetimeevents tt
    on ie.icustay_id = tt.icustay_id
    and tt.charttime between  (ie.first_vent_starttime - interval '6' hour + interval '7 days') and (ie.first_vent_starttime + interval '8 days')
    and itemid in
    (
