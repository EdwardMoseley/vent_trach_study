...
DROP MATERIALIZED VIEW IF EXISTS bloodgasseventhventday CASCADE;
create materialized view bloodgasseventhventday as
with pvt as
( -- begin query that extracts the data
  ...
  -- this also fuses together multiple ITEMIDs containing the same data
    ...
    left join labevents le
      on le.subject_id = tc.subject_id and le.hadm_id = tc.hadm_id
    -- -- 6hr fuzziness
      and le.charttime between (tc.first_vent_starttime - interval '6' hour + interval '7 days') and (tc.first_vent_starttime + interval '8 days')
    -- ---
