...
DROP MATERIALIZED VIEW IF EXISTS weightseventhventday CASCADE;
CREATE MATERIALIZED VIEW weightseventhventday as
with ce as
(
    SELECT
      c.icustay_id
    ...
    FROM chartevents c
    inner join trach_cohort ie
        on c.icustay_id = ie.icustay_id
        and c.charttime <= ie.first_vent_starttime + interval '1' day + interval '7 days'
        and c.charttime > ie.first_vent_starttime - interval '1' day + interval '7 days' -- some fuzziness for admit time
...
(
    ...
    inner join trach_cohort ie
        ...
        and ec.charttime < ie.first_vent_starttime + interval '1' day + interval '7 days'
    where
...
(
    ...
    inner join trach_cohort ie
        ...
        and ie.first_vent_starttime < ec.charttime + interval '1' month
        and ie.first_vent_starttime > ec.charttime
    where
            ec.HADM_ID is null
        and ec.weight is not null
    group by ie.icustay_id
...
inner join patients pat
    ...
    and ie.first_vent_starttime > pat.dob + interval '1' year
