---
title: "Vent Trach Study Cohort Selection"
author: "HST.953"
date: "4/18/2019"
...
`MIMIC-III` flat files must be in the repository folder to run this script. MIMIC-III derived tables, like `icustay_detail.csv` and `ventdurations.csv` will also be used.
...
adm <- read.csv("~/vent_trach_study/MIMIC-III/ADMISSIONS.csv", 
                header = T, stringsAsFactors = F)

## Load PATIENTS Table to join on SUBJECT_ID
pat <- read.csv("~/vent_trach_study/MIMIC-III/PATIENTS.csv", 
                header = T, stringsAsFactors = F)

## Load ICUSTAYS Table to join on SUBJECT_ID, HADM_ID
stays <- read.csv("~/vent_trach_study/MIMIC-III/ICUSTAYS.csv", 
                  header = T, stringsAsFactors = F)

## Load icustay_detail to join with ICUSTAYS
stays_detail <- read.csv("~/vent_trach_study/MIMIC-III/icustay_detail.csv", 
                         header = T, stringsAsFactors = F)
...
```{r}
icd <- merge(read.csv("~/vent_trach_study/MIMIC-III/DIAGNOSES_ICD.csv", header = T, stringsAsFactors = F), read.csv("~/vent_trach_study/MIMIC-III/D_ICD_DIAGNOSES.csv", header = T, stringsAsFactors = F), by = "ICD9_CODE")
...
We are unsure of the mechanical ventilation history of these patients, so will remove them.

```{r}
tmp <- dat[!(dat$ADMISSION_LOCATION == "TRANSFER FROM OTHER HEALT"), ] 

## Numbers
cat(length(unique(dat$SUBJECT_ID)) - length(unique(tmp$SUBJECT_ID)), "patients removed as being transferred from other health facility.\n")
cat(length(unique(dat$HADM_ID)) - length(unique(tmp$HADM_ID)), "hospital admissions associated with transfers from other health facilities.\n")
cat(length(unique(dat$ICUSTAY_ID)) - length(unique(tmp$ICUSTAY_ID)), "ICU stays associated with organ donors.\n")
...
1. Load `ventdurations.csv` (MIMIC-III derived table)
2. Merge data, thus removing any patients which have no ventilation events

```{r}
## (1)
## ventdurations
vent <- read.csv("~/vent_trach_study/MIMIC-III/ventdurations.csv", header = T, stringsAsFactors = F)
...
### Time on Ventilator Selection

1. Convert data from character to dates for ordering, calculate days until death
2. Order based on `SUBJECT_ID`, `HADM_ID`, `ICUSTAY_ID`, `INTIME`, `STARTTIME`
3. Add Cohort information and merge
...
## Days Until Death if the patient expired
...
```{r}
## Our data is contained in tmpd7
dat <- tmpd7
...
dat <- dat[,c("SUBJECT_ID", 
              ...
              "OUTTIME",
              "DISCHTIME", colnames(dat)[!(colnames(dat) %in% c("SUBJECT_ID", 
                                                                "HADM_ID", 
                                                                "ICUSTAY_ID", 
                                                                "ADMITTIME", 
                                                                "INTIME", 
...
## Add 3mo and 1yr mortality RELATIVE TO DISCHARGE
dat$MORTALITY_3MO_DISCH <- (as.numeric(difftime(dat$DOD, dat$DISCHTIME, tz = "EST", units = "days")) <= 90)
dat$MORTALITY_3MO_DISCH <- ifelse(is.na(dat$MORTALITY_3MO_DISCH), FALSE, dat$MORTALITY_3MO_DISCH)

dat$MORTALITY_1YR_DISCH <- (as.numeric(difftime(dat$DOD, dat$DISCHTIME, tz = "EST", units = "days")) <= 365)
dat$MORTALITY_1YR_DISCH <- ifelse(is.na(dat$MORTALITY_1YR_DISCH), FALSE, dat$MORTALITY_1YR_DISCH)

## Overall Days
dat$DAYS_UNTIL_DEATH_DISCH <- as.numeric(difftime(dat$DOD, dat$DISCHTIME, tz = "EST", units = "days"))


## For indexing in psql
tmp <- dat[,c("SUBJECT_ID", 
              "HADM_ID", 
              "ICUSTAY_ID", 
              "ADMITTIME", 
              "INTIME", 
...
## Remove duplicates (some variables contain different data that is unnecessary here)
tmp <- tmp[!duplicated(tmp),]

## Save Cohort
write.csv(dat, file = "~/vent_trach_study/data/final_trach_cohort.csv", row.names = F)
write.csv(tmp, file = "~/vent_trach_study/data/psql_keys.csv", row.names = F)
```
